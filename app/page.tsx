...
export default function QuitoFreshVersionFinal() {
  ...
  const sendWhatsApp = () => {
    ...
    window.open(`https://wa.me/593995849214?text=${message}`, '_blank'); // Reemplaza con tu número real
  };
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', color: '#1A1A1A', fontFamily: 'Inter, sans-serif', position: 'relative', overflowX: 'hidden' }}>
      ...
      {/* ANIMACIONES DE NATURALEZA (Viento) */}
      <style dangerouslySetInnerHTML={{ __html: `
        @keyframes wind { 0%, 100% { transform: rotate(-5deg) translate(0,0); } 50% { transform: rotate(5deg) translate(10px, -5px); } }
        .leaf-decor { position: fixed; opacity: 0.15; animation: wind 7s infinite ease-in-out; pointer-events: none; z-index: 0; }
        .hibiscus { position: fixed; width: 60px; height: 60px; border: 3px solid #E91E63; border-radius: 50%; opacity: 0.1; animation: wind 9s infinite linear; }
      ...
      {/* ELEMENTOS DE FONDO */}
      <div className="leaf-decor" style={{ top: '10%', left: '2%', width: '100px', height: '150px', background: '#8CC63F', borderRadius: '5% 95%' }} />
      <div className="leaf-decor" style={{ top: '60%', right: '2%', width: '80px', height: '120px', background: '#2E7D32', borderRadius: '95% 5%', animationDelay: '2s' }} />
      <div className="hibiscus" style={{ top: '30%', right: '10%' }} />
      ...
      {/* ICONS (Estilo Imagen 2) */}
      <section style={{ display: 'flex', justifyContent: 'center', gap: '40px', padding: '40px 20px', flexWrap: 'wrap' }}>
      ...
      {/* CORPORATE INFO */}
      <section style={{ padding: '40px 20px', maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
