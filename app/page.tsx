...
export default function QuitoFreshFinalBet() {
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', color: '#1A1A1A', fontFamily: 'sans-serif', margin: 0, position: 'relative', overflowX: 'hidden' }}>
      ...
      {/* CAPA DE DISEÑO: HOJAS Y SPLASHES TIPO LOGO */}
      <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-10%', right: '-5%', width: '600px', height: '600px', background: 'radial-gradient(circle, rgba(255, 180, 0, 0.15) 0%, transparent 70%)', borderRadius: '50%' }} />
        <div style={{ position: 'absolute', top: '35%', left: '-10%', width: '700px', height: '700px', background: 'radial-gradient(circle, rgba(140, 198, 63, 0.1) 0%, transparent 70%)', borderRadius: '50%' }} />
        
        {/* Hojas con la punta del logo */}
        <div style={{ position: 'absolute', top: '15%', left: '5%', width: '100px', height: '140px', background: '#8CC63F', opacity: 0.18, borderRadius: '5% 95% 5% 95%', transform: 'rotate(-20deg)' }} />
        <div style={{ position: 'absolute', top: '60%', right: '3%', width: '80px', height: '120px', background: '#2E7D32', opacity: 0.12, borderRadius: '95% 5% 95% 5%', transform: 'rotate(15deg)' }} />
      </div>

      <style dangerouslySetInnerHTML={{ __html: `
        .text-bold { font-weight: 900; text-transform: uppercase; letter-spacing: -0.5px; }
        .info-card { background: rgba(255,255,255,0.8); backdrop-filter: blur(10px); padding: 40px; border-radius: 35px; border: 1px solid #F0F0F0; text-align: left; transition: 0.3s; }
        .info-card:hover { transform: translateY(-5px); border-color: #8CC63F; }
        ...
        .featured { border: 3px solid #E91E63; transform: scale(1.04); }
      ...
      {/* NAV */}
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(10px)', borderBottom: '1px solid #F0F0F0' }}>
      ...
      <header style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '70px 20px' }}>
        <div style={{ border: '2px solid #8CC63F', color: '#8CC63F', display: 'inline-block', padding: '6px 18px', borderRadius: '50px', fontWeight: '900', fontSize: '13px', marginBottom: '25px' }}>EST. 2026</div>
        ...
        <img src="/1000786704.jpg" alt="Hero Logo" style={{ maxWidth: '300px', margin: '40px 0', filter: 'drop-shadow(0 20px 30px rgba(0,0,0,0.1))' }} />
      </header>

      {/* SECCIÓN INFORMACIÓN CORPORATIVA (RESTAURADA) */}
      <section style={{ position: 'relative', zIndex: 1, padding: '40px 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px', maxWidth: '1200px', margin: '0 auto' }}>
        <div className="info-card">
          ...
          <p style={{ lineHeight: '1.7', color: '#555' }}>Nutrir con extractos puros de la tierra andina, fomentando un estilo de vida consciente y lleno de energía vital para tu día a día.</p>
        </div>
        <div className="info-card" style={{ background: '#8CC63F', color: 'white' }}>
          <h3 className="text-bold" style={{ marginBottom: '15px' }}>Desde 2026</h3>
          <p style={{ lineHeight: '1.7' }}>Nacemos con la promesa de llevar la frescura absoluta del campo directamente a tu mano, sin aditivos, sin engaños, solo fruta pura.</p>
        </div>
        <div className="info-card">
          <h3 className="text-bold" style={{ color: '#8CC63F', marginBottom: '15px' }}>Nuestra Visión</h3>
          <p style={{ lineHeight: '1.7', color: '#555' }}>Ser líderes en bienestar premium en Ecuador, reconocidos por la calidad inigualable de nuestros jugos con tecnología Cold Pressed.</p>
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '40px' }}>
          {products.map(p => (
            <div key={p.id} className={`product-card ${p.id === 1 ? 'featured' : ''}`} style={{ opacity: p.disabled ? 0.6 : 1 }}>
              ...
              <div style={{ fontSize: '2.8rem', fontWeight: '900', margin: '20px 0', color: '#1A1A1A' }}>${p.price.toFixed(2)}</div>
              <button disabled={p.disabled} onClick={() => addToCart(p)} className="btn-green" style={{ width: '100%', background: p.accent }}>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '60px 20px 100px 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '40px', maxWidth: '1000px', margin: '0 auto', textAlign: 'center' }}>
          <div>
              <div style={{ fontSize: '50px', marginBottom: '15px' }}>⚙️</div>
              <h3 className="text-bold" style={{ fontSize: '1.2rem' }}>PRENSADO EN FRÍO</h3>
              <p style={{ color: '#666', fontSize: '14px' }}>Máxima Nutrición y Sabor Real.</p>
          </div>
          <div>
              <div style={{ fontSize: '50px', marginBottom: '15px' }}>🏔️</div>
              <h3 className="text-bold" style={{ fontSize: '1.2rem' }}>ORIGEN ANDINO</h3>
              <p style={{ color: '#666', fontSize: '14px' }}>Fruta de los Valles de Quito.</p>
          </div>
          <div>
              <div style={{ fontSize: '50px', marginBottom: '15px' }}>🌿</div>
              <h3 className="text-bold" style={{ fontSize: '1.2rem' }}>100% NATURAL</h3>
              <p style={{ color: '#666', fontSize: '14px' }}>Sin Azúcar ni preservantes.</p>
          </div>
      </section>

      {/* FOOTER */}
      <footer style={{ padding: '60px 40px', textAlign: 'center', background: '#111', color: 'white' }}>
        <img src="/1000786704.jpg" alt="Logo" style={{ height: '50px', marginBottom: '20px', filter: 'brightness(1.5)' }} />
        <div className="text-bold">QUITO FRESH © 2026</div>
        <div style={{ marginTop: '40px', fontSize: '9px', opacity: 0.3, letterSpacing: '2px' }}>DESARROLLADO POR ECOS BRANDING & ORCA STUDIOS</div>
      </footer>

      {/* CARRITO CON ELIMINACIÓN INDIVIDUAL */}
      <div className="sidebar">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          <h2 className="text-bold" style={{ fontSize: '1.5rem' }}>TU PACK</h2>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {cart.length === 0 && <p style={{ textAlign: 'center', color: '#999', marginTop: '50px' }}>El pack está vacío.</p>}
        </div>
        ...
        {cart.length > 0 && (
          <div style={{ borderTop: '2px solid #EEE', paddingTop: '20px' }}>
            ...
            <button onClick={() => setCart([])} style={{ width: '100%', background: 'none', border: '1px solid #FF5252', color: '#FF5252', padding: '12px', borderRadius: '50px', marginBottom: '15px', fontWeight: 'bold' }}>VACIAR TODO</button>
            <button style={{ width: '100%', background: '#25D366', color: 'white', border: 'none', padding: '18px', borderRadius: '50px', fontWeight: '900' }}>PEDIR POR WHATSAPP 📱</button>
          </div>
        )}
      </div>
    </div>
